...
module.exports = {
  ...
  fetch: (params) => {
    ...
    return Blogposts
      .findOne(_.pick(params, _.keys(Blogposts.schema.paths)))
      .populate(populate);
  },
  ...
  add: async (values) => {
    ...
    const entry = await Blogposts.create(data);

    // Create relational data and return the entry.
    return Blogposts.updateRelations({ _id: entry.id, values: relations });
  },
  ...
  edit: async (params, values) => {
    // Extract values related to relational data.
    const relations = _.pick(values, Blogposts.associations.map(a => a.alias));
    const data = _.omit(values, Blogposts.associations.map(a => a.alias));

    // Update entry with no-relational data.
    const entry = await Blogposts.update(params, data, { multi: true });
  ...
  remove: async params => {
    ...
    await Promise.all(
      Blogposts.associations.map(async association => {
  ...
  search: async (params) => {
    ...
    const $or = Object.keys(Blogposts.attributes).reduce((acc, curr) => {
      switch (Blogposts.attributes[curr].type) {
        case 'integer':
        case 'float':
        case 'decimal':
